...
class TeamworkDeviceOperation extends Entity implements Parsable 
{
    ...
    */
    public function getLastActionBy(): ?IdentitySet {
        $val = $this->getBackingStore()->get('lastActionBy');
        if (is_null($val) || $val instanceof IdentitySet) {
            return $val;
        }
        throw new \UnexpectedValueException("Invalid type found in backing store for 'lastActionBy'");
    }
    ...
    */
    public function getOperationType(): ?TeamworkDeviceOperationType {
        ...
        if (is_null($val) || $val instanceof TeamworkDeviceOperationType) {
            return $val;
        }
