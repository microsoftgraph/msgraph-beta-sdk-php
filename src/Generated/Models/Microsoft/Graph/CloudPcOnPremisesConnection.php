...
class CloudPcOnPremisesConnection extends Entity 
{
    ...
    /** @var string|null $displayName The display name for the on-premises connection. */
    private ?string $displayName = null;
    
    /** @var CloudPcOnPremisesConnectionStatus|null $healthCheckStatus The status of the most recent health check done on the on-premises connection. For example, if status is 'passed', the on-premises connection has passed all checks run by the service. Possible values are: pending, running, passed, failed, unknownFutureValue. Read-only. */
    ...
    /** @var bool|null $inUse When true, the on-premises connection is in use. When false, the connection is not in use. You cannot delete a connection that’s in use. Returned only on $select. For an example that shows how to get the inUse property, see Example 2: Get the selected properties of an on-premises connection, including healthCheckStatusDetails. Read-only. */
