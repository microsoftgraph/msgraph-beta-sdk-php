...
class Incident extends Entity implements Parsable 
{
    ...
    */
    public function getRecommendedHuntingQueries(): ?array {
        $val = $this->getBackingStore()->get('recommendedHuntingQueries');
        if (is_array($val) || is_null($val)) {
            TypeUtils::validateCollectionValues($val, RecommendedHuntingQuery::class);
            /** @var array<RecommendedHuntingQuery>|null $val */
            return $val;
        }
        throw new \UnexpectedValueException("Invalid type found in backing store for 'recommendedHuntingQueries'");
    }
    ...
    */
    public function setRecommendedHuntingQueries(?array $value): void {
        $this->getBackingStore()->set('recommendedHuntingQueries', $value);
    }
