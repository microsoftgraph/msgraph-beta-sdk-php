...
class EducationIdentityCreationConfiguration extends EducationIdentitySynchronizationConfiguration implements Parsable 
{
    ...
    */
    public function getUserDomains(): ?array {
        $val = $this->getBackingStore()->get('userDomains');
        if (is_array($val) || is_null($val)) {
            TypeUtils::validateCollectionValues($val, EducationIdentityDomain::class);
            /** @var array<EducationIdentityDomain>|null $val */
            return $val;
        }
        throw new \UnexpectedValueException("Invalid type found in backing store for 'userDomains'");
    }
