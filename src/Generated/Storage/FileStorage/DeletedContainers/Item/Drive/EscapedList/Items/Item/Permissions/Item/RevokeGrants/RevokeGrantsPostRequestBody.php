...
class RevokeGrantsPostRequestBody implements AdditionalDataHolder, BackedModel, Parsable 
{
    /**
    ...
    */
    public function getGrantees(): ?array {
        $val = $this->getBackingStore()->get('grantees');
        if (is_array($val) || is_null($val)) {
            TypeUtils::validateCollectionValues($val, DriveRecipient::class);
            /** @var array<DriveRecipient>|null $val */
            return $val;
        }
        throw new \UnexpectedValueException("Invalid type found in backing store for 'grantees'");
    }
    ...
    */
    public function setGrantees(?array $value): void {
        $this->getBackingStore()->set('grantees', $value);
    }
