...
class MeetingRegistrationBase extends Entity
{
    ...
    * Registrants of the online meeting.
     *
     * @return MeetingRegistrantBase[]|null The registrants
     */
    public function getRegistrants()
    {
        if (array_key_exists('registrants', $this->_propDict) && !is_null($this->_propDict['registrants'])) {
            $registrants = [];
            if (count($this->_propDict['registrants']) > 0 && is_a($this->_propDict['registrants'][0], 'MeetingRegistrantBase')) {
                return $this->_propDict['registrants'];
            }
            foreach ($this->_propDict['registrants'] as $singleValue) {
                $registrants []= new MeetingRegistrantBase($singleValue);
            }
