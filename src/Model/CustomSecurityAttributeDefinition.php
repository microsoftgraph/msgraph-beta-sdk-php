...
class CustomSecurityAttributeDefinition extends Entity
{
    ...
    */
    ...
    {
        if (array_key_exists('allowedValues', $this->_propDict) && !is_null($this->_propDict['allowedValues'])) {
            $allowedValues = [];
            if (count($this->_propDict['allowedValues']) > 0 && is_a($this->_propDict['allowedValues'][0], 'AllowedValue')) {
                return $this->_propDict['allowedValues'];
            }
            foreach ($this->_propDict['allowedValues'] as $singleValue) {
                $allowedValues []= new AllowedValue($singleValue);
            }
            $this->_propDict['allowedValues'] = $allowedValues;
            return $this->_propDict['allowedValues'];
        }
        return null;
    }
