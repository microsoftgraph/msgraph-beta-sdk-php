...
class TeamworkSpeakerConfiguration extends Entity
{
    ...
    */
    ...
    {
        if (array_key_exists("defaultCommunicationSpeaker", $this->_propDict) && !is_null($this->_propDict["defaultCommunicationSpeaker"])) {
     
            if (is_a($this->_propDict["defaultCommunicationSpeaker"], "\Beta\Microsoft\Graph\Model\TeamworkPeripheral")) {
    ...
    */
    ...
    {
        if (array_key_exists("defaultSpeaker", $this->_propDict) && !is_null($this->_propDict["defaultSpeaker"])) {
     
            if (is_a($this->_propDict["defaultSpeaker"], "\Beta\Microsoft\Graph\Model\TeamworkPeripheral")) {
    ...
    */
    public function getSpeakers()
    {
        if (array_key_exists("speakers", $this->_propDict) && !is_null($this->_propDict["speakers"])) {
            ...
            if (count($this->_propDict['speakers']) > 0 && is_a($this->_propDict['speakers'][0], 'TeamworkPeripheral')) {
                return $this->_propDict['speakers'];
            }
            $speakers = [];
            foreach ($this->_propDict['speakers'] as $singleValue) {
                $speakers []= new TeamworkPeripheral($singleValue);
            }
