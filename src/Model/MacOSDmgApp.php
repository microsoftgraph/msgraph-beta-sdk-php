...
class MacOSDmgApp extends MobileLobApp
{
    ...
    * The list of apps expected to be installed by the DMG.
     *
     * @return MacOSIncludedApp[]|null The includedApps
     */
    public function getIncludedApps()
    {
        if (array_key_exists('includedApps', $this->_propDict) && !is_null($this->_propDict['includedApps'])) {
            $includedApps = [];
            if (count($this->_propDict['includedApps']) > 0 && is_a($this->_propDict['includedApps'][0], 'MacOSIncludedApp')) {
                return $this->_propDict['includedApps'];
            }
            foreach ($this->_propDict['includedApps'] as $singleValue) {
                $includedApps []= new MacOSIncludedApp($singleValue);
            }
            $this->_propDict['includedApps'] = $includedApps;
            return $this->_propDict['includedApps'];
        }
        return null;
    }
