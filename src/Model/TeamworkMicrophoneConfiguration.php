...
class TeamworkMicrophoneConfiguration extends Entity
{
    ...
    */
    ...
    {
        if (array_key_exists("defaultMicrophone", $this->_propDict) && !is_null($this->_propDict["defaultMicrophone"])) {
     
            if (is_a($this->_propDict["defaultMicrophone"], "\Beta\Microsoft\Graph\Model\TeamworkPeripheral")) {
    ...
    */
    ...
    {
        if (array_key_exists("microphones", $this->_propDict) && !is_null($this->_propDict["microphones"])) {
       
            if (count($this->_propDict['microphones']) > 0 && is_a($this->_propDict['microphones'][0], 'TeamworkPeripheral')) {
                return $this->_propDict['microphones'];
            }
            $microphones = [];
            foreach ($this->_propDict['microphones'] as $singleValue) {
                $microphones []= new TeamworkPeripheral($singleValue);
            }
            $this->_propDict['microphones'] = $microphones;
            return $this->_propDict['microphones'];
            }
        return null;
    }
