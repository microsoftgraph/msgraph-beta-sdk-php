...
class TeamworkDisplayConfiguration extends Entity
{
    ...
    */
    ...
    {
        if (array_key_exists("configuredDisplays", $this->_propDict) && !is_null($this->_propDict["configuredDisplays"])) {
       
            if (count($this->_propDict['configuredDisplays']) > 0 && is_a($this->_propDict['configuredDisplays'][0], 'TeamworkConfiguredPeripheral')) {
                return $this->_propDict['configuredDisplays'];
            }
            $configuredDisplays = [];
            foreach ($this->_propDict['configuredDisplays'] as $singleValue) {
                $configuredDisplays []= new TeamworkConfiguredPeripheral($singleValue);
            }
            $this->_propDict['configuredDisplays'] = $configuredDisplays;
            return $this->_propDict['configuredDisplays'];
            }
        return null;
    }
    ...
    */
    ...
    {
        if (array_key_exists("inBuiltDisplayScreenConfiguration", $this->_propDict) && !is_null($this->_propDict["inBuiltDisplayScreenConfiguration"])) {
     
            if (is_a($this->_propDict["inBuiltDisplayScreenConfiguration"], "\Beta\Microsoft\Graph\Model\TeamworkDisplayScreenConfiguration")) {
