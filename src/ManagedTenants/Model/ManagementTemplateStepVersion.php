...
class ManagementTemplateStepVersion extends \Beta\Microsoft\Graph\Model\Entity
{
    ...
    */
    ...
    {
        if (array_key_exists("configurationAction", $this->_propDict) && !is_null($this->_propDict["configurationAction"])) {
            if (is_a($this->_propDict["configurationAction"], "\Beta\Microsoft\Graph\ManagedTenants\Model\TemplateAction")) {
    ...
    */
    ...
    {
        if (array_key_exists("validationAction", $this->_propDict) && !is_null($this->_propDict["validationAction"])) {
            if (is_a($this->_propDict["validationAction"], "\Beta\Microsoft\Graph\ManagedTenants\Model\TemplateAction")) {
    ...
    */
     ...
     *
     * @return ManagementTemplateStepDeployment[]|null The deployments
     */
    public function getDeployments()
    {
        if (array_key_exists('deployments', $this->_propDict) && !is_null($this->_propDict['deployments'])) {
            $deployments = [];
            if (count($this->_propDict['deployments']) > 0 && is_a($this->_propDict['deployments'][0], 'ManagementTemplateStepDeployment')) {
                return $this->_propDict['deployments'];
            }
            foreach ($this->_propDict['deployments'] as $singleValue) {
                $deployments []= new ManagementTemplateStepDeployment($singleValue);
            }
    ...
    */
    ...
    {
        if (array_key_exists("templateStep", $this->_propDict) && !is_null($this->_propDict["templateStep"])) {
            if (is_a($this->_propDict["templateStep"], "\Beta\Microsoft\Graph\ManagedTenants\Model\ManagementTemplateStep")) {
